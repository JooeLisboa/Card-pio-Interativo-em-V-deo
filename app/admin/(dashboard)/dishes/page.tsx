...
export default async function AdminDishesPage({
  ...
  const dishCards = await Promise.all(
    data.dishes.map(async (dish) => {
      ...
      return (
        <Card
          key={dish.id}
          className="grid gap-4 p-5 md:grid-cols-[120px_1fr_200px] md:items-center"
        >
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        title="Pratos"
        description="Cadastre pratos com URL própria, vídeo opcional, status e QR individual."
      />
