  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <MetricCard label="Visualizações totais" value={data.totalViews} description="Soma dos acessos válidos registrados nas páginas dos pratos." />
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_360px]">
        <Card className="p-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <Link href="/admin/analytics" className="text-sm font-semibold text-[var(--primary)]">
              Ver analytics
            </Link>
          </div>
          <div className="mt-6 grid gap-4">
            {data.topDishes.length ? (
              data.topDishes.map((dish, index) => (
                <div key={dish.id} className="flex items-center justify-between gap-4 rounded-2xl border border-[var(--border)] p-4">
                  <div>
              ...
              <div className="rounded-2xl border border-dashed border-[var(--border)] p-6 text-sm text-stone-500">
                Ainda não há visualizações registradas. Abra um prato público para validar o fluxo de analytics.
              </div>
            )}
          </div>
        </Card>
        <Card className="p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <a href={qrCode} download={`qr-cardapio-${restaurantSlug}.png`} className="text-sm font-semibold text-[var(--primary)]">
              Baixar QR
            </a>
            <Link href={`/menu/${restaurantSlug}`} className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--primary)]">
